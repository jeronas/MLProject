...
#x_unl = data_unl.iloc[:, :-1]
#y_unl = data_unl.iloc[:, 15]
...
#print(x_train)
#x_unl[0].fillna(x_unl[0].mediann(), inplace =True)
#print(x_unl)
...
X_train = pd.get_dummies(x_train,columns=[2], prefix=[2])
X1_train = pd.get_dummies(X_train,columns=[4], prefix=[4])
X2_train = pd.get_dummies(X1_train,columns=[7], prefix=[7])
X3_train = pd.get_dummies(X2_train,columns=[8], prefix=[8])
X4_train = pd.get_dummies(X3_train,columns=[14], prefix=[14])
...
clfDT.fit(X4_train, y_train)
clfNN.fit(X4_train, y_train)
clfSVM.fit(X4_train, y_train)
clfNB.fit(X4_train, y_train)
clfRF.fit(X4_train, y_train)
